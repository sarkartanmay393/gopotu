...
class ShowUpAnimationPage extends StatefulWidget {
  ...
  final Widget child;
  final int? delay; // Making delay optional by using int?

  const ShowUpAnimationPage({
    this.duration = const Duration(seconds: 1), // Providing a default value for duration
    required this.child,
    this.delay, // Making delay optional
  });
...
class _ShowUpAnimationPage extends State<ShowUpAnimationPage> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _animOffset = Tween<Offset>(begin: const Offset(0.0, -0.05), end: Offset.zero).animate(curve);
